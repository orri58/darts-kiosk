...
export default function OperatorLayout() {
  ...
  if (canManage) {
    NAV_ITEMS.push({ path: '/operator/customers', icon: Building2, label: 'Kunden', tid: 'op-nav-customers' });
  }
  NAV_ITEMS.push({ path: '/operator/locations', icon: MapPin, label: 'Standorte', tid: 'op-nav-locations' });
  NAV_ITEMS.push({ path: '/operator/devices', icon: Monitor, label: 'Geräte', tid: 'op-nav-devices' });
  if (canManage) {
    NAV_ITEMS.push({ path: '/operator/licenses', icon: KeyRound, label: 'Lizenzen', tid: 'op-nav-licenses' });
  }
  if (canManageStaff) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950" data-testid="operator-layout">
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between px-4 h-14 z-50" data-testid="op-mobile-header">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5" data-testid="op-nav">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
