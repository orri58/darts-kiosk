...
export default function AdminSettings() {
  ...
  const [customizationImportJson, setCustomizationImportJson] = useState('');
  ...
  const handleExportCustomizationProfile = async () => {
    setCustomizationBusy(true);
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const res = await axios.get(`${API}/settings/customization-profile/export`, { headers });
      const pretty = JSON.stringify(res.data, null, 2);
      setCustomizationImportJson(pretty);
      const blob = new Blob([pretty], { type: 'application/json;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `darts-kiosk-customization-profile-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      toast.success('Customization-Profil exportiert');
    } catch (error) {
  ...
  const handleCustomizationImportFile = async (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    try {
      const text = await file.text();
      setCustomizationImportJson(text);
  ...
  const handleImportCustomizationProfile = async () => {
    setCustomizationBusy(true);
    try {
      const parsed = JSON.parse(customizationImportJson || '{}');
      const payload = parsed?.bundle ? parsed : { bundle: parsed };
      const headers = { Authorization: `Bearer ${token}` };
      await axios.post(`${API}/settings/customization-profile/import`, payload, { headers });
      await refreshSettings();
      toast.success('Customization-Profil importiert');
    } catch (error) {
  ...
  const handleResetCustomizationProfile = async () => {
    if (!window.confirm('Wirklich alle kiosk/admin Customization-Einstellungen auf Standard zurücksetzen?')) return;
    setCustomizationBusy(true);
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.post(`${API}/settings/customization-profile/reset`, { confirm: true }, { headers });
      await refreshSettings();
      setCustomizationImportJson('');
      toast.success('Customization-Profil zurückgesetzt');
    } catch (error) {
    ...
    >
      <div data-testid="admin-settings" className="space-y-6">
        ...
        <TabsList className="flex h-auto flex-nowrap gap-1 overflow-x-auto rounded-2xl border border-[rgb(var(--color-border-rgb)/0.82)] bg-[rgb(var(--color-surface-rgb)/0.66)] p-1">
          ...
          <TabsTrigger value="customization-profile" data-testid="tab-customization-profile" className="data-[state=active]:bg-amber-500 data-[state=active]:text-black">
