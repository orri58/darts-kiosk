...
if exist "app\backend\.env" call :load_env_value BOARD_ID BOARD_ID
if exist "app\.venv\Scripts\activate.bat" call "app\.venv\Scripts\activate.bat"

python app\bin\local_smoke.py --base-url "%BASE_URL%" --board-id "%BOARD_ID%"
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if "%EXIT_CODE%"=="0" (
    echo [OK] Runtime Smoke-Test erfolgreich.
) else (
    echo [FAIL] Runtime Smoke-Test fehlgeschlagen. Bitte data\logs\backend.log pruefen.
)
echo.
pause
