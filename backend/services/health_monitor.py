...
Tracks system health, observer status, and agent connectivity
"""
import asyncio
import logging
...
class ObserverMetrics:
    """Metrics for Autodarts observer"""
    total_events: int = 0
    successful: int = 0
    failed: int = 0
    ...
    def _update_rate(self):
        if self.total_events > 0:
            self.success_rate = round(self.successful / self.total_events * 100, 2)
...
class HealthMonitor:
    ...
    def __init__(self):
        ...
        self._observer_metrics = ObserverMetrics()
        ...
        try:
            async with AsyncSessionLocal() as db:
                result = await db.execute(
                    select(Board).where(Board.agent_api_base_url.is_not(None))
                )
    ...
    def record_observer_event(self, success: bool, error: str = "", screenshot_path: Optional[str] = None):
        ...
        if success:
            self._observer_metrics.record_success()
        else:
            self._observer_metrics.record_failure(error, screenshot_path)
            self._add_error("observer", error, screenshot_path)
